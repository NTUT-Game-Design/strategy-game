let GameMap = function (){
    ...
    this.SetMask = function(){
        this._tiles.forEach(row => {
            row.forEach(element => {
                ...
                sp.scale = 0.15;
                //this.Mask[element.GetIndex().y][element.GetIndex().x] = {sp:sp,val:1};
    ...
    this._GetWorldPos = function(pos){
        return { x:pos.x + this._pos.x - (this._size.x / 2) + this._offset.x,
